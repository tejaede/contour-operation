...
```
# npm run build
docker build . -t contour:develop-SNAPSHOT
...
### Npm commands
 ...
 - `start:stack`: docker stack deploy -c docker-compose.yml 'contour-stack' 
 - `stop:stack`: docker stack rm 'contour-stack' 
 - `stop:swarm`: docker swarm leave --force 
 - `build`: npm run build:docker 
 - `build:docker`: docker build . -t contour:develop-SNAPSHOT 
